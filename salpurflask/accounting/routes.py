...
def reverse_document_route(kind, id):
    ...
    # A Sale can carry CustomerPayments raised against it, and a Purchase
    # can likewise carry SupplierPayments. Reversing either removes only
    # its own ledger/GL/stock effects (see reverse_document /
    # _unwind_stock_and_subledger) -- it deliberately does not touch those
    # payments, so the customer/supplier ledger is left with an active
    # credit until someone reconciles or refunds it separately. That is
    # allowed (Option A), but it must not happen silently: the user has to
    # see the warning and explicitly continue. Checked here, not only in
    # the template, so a hand-built POST can't skip past it.
    if kind == "sale":
        ...
        has_active_payments = any(not p.is_reversed for p in doc.supplier_payments)
        if has_active_payments and request.form.get("confirm_payment_warning") != "1":
            flash(
                "This Purchase has received supplier payment(s). Reversing this Purchase "
                "will remove it from active Purchase Cost and Payables, but the existing "
                "payment(s) will remain recorded. This may create a supplier credit "
                "balance until the payment is refunded or reconciled. Confirm to continue.",
                "warning")
            return redirect(request.referrer or url_for(list_endpoint))
